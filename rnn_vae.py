...
class RNN_VAE:
    ...
    def test_epoch(self, epoch, X_iter, verbatim=False):
        start = timeit.default_timer()
        epoch_loss = 0.0
        num_batches = X_iter.batch_sampler.batch_count()
        for idx, inputs in enumerate(X_iter):
    ...
    def train_batch(self, inputs): # Right now, we assume that inputs and lengths are sorted
        ...
        #print(TextRnnVAE.are_equal_tensors(self.decoder.out.weight, self.encoder.embedding.weight))

        return loss.item() / (num_steps)


    def test_batch(self, inputs): # Right now, we assume that inputs and lengths are sorted
        batch_size, num_steps = inputs.shape
        # Initialize hidden state
        self.encoder.hidden = self.encoder.init_hidden(batch_size)
        # Zero gradients of both optimizers
...
class Decoder(nn.Module):
    ...
    def _step(self, input, hidden):
        ...
        output = self.out(output.squeeze(dim=1)) # F.log_softmax(self.out(output.squeeze(dim=1)), dim=1)
